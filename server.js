...
socket.on("messageSent", async ({ senderId, receiverId }) => {
    await Conversation.updateOne(
      { members: { $all: [senderId, receiverId] } },
      { lastMessageTime: new Date() }
    )

    console.log("Emitting updateContacts event...")
    io.emit("updateContacts") // Emit globally to update all connected clients
  })
